...
import type { InputFrame, InputMode, InputProvider } from "../domain/input/types";
...
function directionLabel(direction: number | undefined): string {
  if (!direction) {
    return "-";
  }
  if (direction === 5) {
...
function frameRangeLabel(startFrame: number, endFrame: number): string {
  if (startFrame === endFrame) {
    return String(startFrame);
  }
                ...
                {visibleHistoryEntries.length > 0 ? (
                  visibleHistoryEntries.map((entry) => (
                    <tr key={`${entry.startFrame}-${entry.endFrame}`}>
                      ...
                      <td>{entry.down.join("+") || "-"}</td>
                      <td>{frameRangeLabel(entry.startFrame, entry.endFrame)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={4}>No input history yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
