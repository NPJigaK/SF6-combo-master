...
### Phase 4: 将来拡張
- [x] コンボの左右入れ替え（キャラの立ち位置が逆になる。つまりコンボ入力が逆向きにする）
- [x] キャラ選択 UI（現在 JP 固定）
- [x] コンボビルダー UI（trial JSON を GUI で作成）
- [x] Zustand 導入（設定が複雑化した場合のみ・現時点では不要）

### Phase 5 UI の再設計
- [x] codex と claude で相談してこのアプリにおけるベストプラクティスなUI設計を考える（破壊的な変更も認める）
